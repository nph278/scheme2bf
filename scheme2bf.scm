...
(define (make-l2 debug?)
  ...
  (define instructions
    '((label . 0) ;; (label expr) -- -
      (halt . 0)  ;; (halt)       -- halt 0 0
      (push . 1)  ;; (push expr)  -- push 0 0   [datum]
      (jump . 2)  ;; (jump label) -- jump 0 address
      ))
  ...
  (define instruction-position -1)
  (define labels '())

  (define (label-address label)
    (or (cdr (assoc label labels))
        (error)))

  (define (compile-instruction expr)
    (set! instruction-position (+ instruction-position 1))
    (cond ((not (list expr))
           (error))
          ((nil? expr)
           (error))
           ...
           `(,@(debug expr)
             ...
             ,@(cond ((case (first expr)
                        ...
                        ((push)
                         ...
                         `((move ,(* 2 address-width))
                           ...
                           ,@(cond ((nil? (drop expr 1))
                                   ...
                                   (else (error)))))
                        ((label)
                         (cond ((nil? (drop expr 1)) (error))
                               ((not (nil? (drop expr 2))) (error))
                               (else (set! labels (alist-cons (second expr)
                        ...
                        ((jump)
                         (cond ((nil? (drop expr 1)) (error))
                               ((not (nil? (drop expr 2))) (error))
                               (else `((move ,(- (* 2 address-width) 1))
                                       (add ,(label-address (second expr)))
                                       (move 1)))))
                        (else (error)))))))))
